 ...
 * LED-Matrix
 */
public class RectLedMatrix extends Node implements Element {

    /**
     * the LED-Matrix description
     */
    public static final ElementTypeDescription DESCRIPTION
            = new ElementTypeDescription(RectLedMatrix.class, input("r-data"), input("c-addr"))
    ...
    private RectLedMatrixDialog RectledMatrixDialog;
     ...
     */
    public RectLedMatrix(ElementAttributes attr) {
    ...
    private void dataChanged(int colAddr, long rowData) {
        if (paintPending.compareAndSet(false, true)) {
            SwingUtilities.invokeLater(() -> {
                if (RectledMatrixDialog == null || !RectledMatrixDialog.isVisible()) {
                    RectledMatrixDialog = new RectLedMatrixDialog(getModel().getWindowPosManager().getMainFrame(), dy, data, color, ledPersist);
                    getModel().getWindowPosManager().register("RectledMatrix_" + label, RectledMatrixDialog);
                }
                paintPending.set(false);
                RectledMatrixDialog.updateGraphic(colAddr, rowData);
            });
        }
    }
}
